...
//        PreferencesHelper.saveData(Constants.CONF_SESSION, session);
...
//        try {
//            PreferencesHelper.remove(Constants.CONF_SESSION);
//        } catch (Exception e) {
//        }
    ...
    public void removeCallSessionFromMap(int callId) {
...
//        }
    ...
    public Session getCallSessionByCallID(int callID) {
        ...
        session = callSessionMap.get(callID);
//        if (null == session) {
//            try {
//                session = PreferencesHelper.getData(Constants.CONF_SESSION, Session.class);
//            } catch (Exception e) {
//            }
//        }
        return session;
...
//        int solution = LoginCenter.getInstance().getSolution();
        ...
        if (LoginCenter.getInstance().getServerType() == LoginCenter.getInstance().LOGIN_E_SERVER_TYPE_SMC) {
            ...
            if (call != null) {
                ...
                LogUtil.i(UIConstants.DEMO_TAG, "CallMgr-->236");
                ...
                LogUtil.i(TAG, "access reserved conf is success.");
    ...
    public synchronized int startCall(String toNumber, boolean isVideoCall) {
        ...
        if (call != null) {
            Session newSession = new Session(call);
            LogUtil.i(UIConstants.DEMO_TAG, "CallMgr-->272");
            putCallSessionToMap(newSession);
    ...
    public boolean muteMic(int callID, boolean mute) {
        Session callSession = getCallSessionByCallID(callID);
        if (callSession == null) {
//            throw new ServerException("callSession不能为空", -1);
            return false;
        }
...
//            throw new ServerException("callSession不能为空", -1);
     ...
     * @return
     */
    public boolean getMuteSpeakStatus(int callID) {
        Session callSession = getCallSessionByCallID(callID);
        if (callSession == null) {
//            throw new ServerException("callSession不能为空", -1);
            return false;
        }
        return callSession.getMuteSpeakStatus();
    }
     ...
     * @param mute
     */
    public void setMuteSpeakStatus(int callID, boolean mute) {
        Session callSession = getCallSessionByCallID(callID);
        if (callSession == null) {
//            throw new ServerException("callSession不能为空", -1);
            return;
        }
        callSession.setMuteSpeakStatus(mute);
    }
...
//            throw new ServerException("callSession不能为空", -1);
     ...
     * 播放铃音
     *
     * @param ringingFile 来电播放铃声
     */
    @Override
    public void startPlayRingingTone(String ringingFile, int callId) {
        int result;
...
//            @Override
//            public void run() {
//                //判断是否在来电界面，如果不在来电界面和会议界面则挂断
//                Activity activity = AppManager.getInstance().currentActivity();
//                LogUtil.i(UIConstants.DEMO_TAG, "播放五秒后:" + activity.getPackageName());
//                if (null != activity) {
//                    if (!(activity instanceof CallerIDActivity)//不属于来电界面
//                            && !(activity instanceof VideoActivity)//不属于视频界面
//                            && !(activity instanceof VideoConfActivity)) {
...
//        callManager.setMobileAudioRoute(TupCallParam.CALL_E_MOBILE_AUIDO_ROUTE.CALL_MOBILE_AUDIO_ROUTE_LOUDSPEAKER);
    ...
    private void handleCallComing(TupCall tupCall) {
        ...
        LogUtil.i(UIConstants.DEMO_TAG, "CallMgr-->保存新的session数据");
    ...
    private void handleCallVideoModifyResult(TupCall call) {
        ...
        LogUtil.i(TAG, "result: " + result + "isVideo: " + isVideo + "callId: " + callId);
        ...
        if (result == 0) {
            //video --> audio success
            if (isVideo == 0) {
                LogUtil.i(TAG, "Video to audio Call");
                //VideoDeviceManager.getInstance().clearCallVideo();
                VideoMgr.getInstance().clearCallVideo();
    ...
    public void onCallViedoResult(TupCall call) {
        LogUtil.i(UIConstants.DEMO_TAG, "onCallViedoResult");
        if (null == call) {
    ...
    public void onSessionModified(TupCall tupCall) {
        ...
        LogUtil.i(UIConstants.DEMO_TAG, "onSessionModified");
        LogUtil.i(TAG, "conf id:" + tupCall.getServerConfID());
    ...
    public void onConfNotify(int code, Conf conf) {
        LogUtil.i(TAG, "onConfNotify eventType->" + code);
    ...
    public void onCallBldTransferRecvSucRsp(TupCall tupCall) {
        LogUtil.i(UIConstants.DEMO_TAG, "onCallUnHoldSuccess");
    ...
    public void onCallComing(TupCall tupCall) {
        LogUtil.i(UIConstants.DEMO_TAG, "onCallComing");
        if (null == tupCall) {
    ...
    public void onCallRingBack(TupCall tupCall) {
        LogUtil.i(UIConstants.DEMO_TAG, "onCallRingBack");
        if (null == tupCall) {
        ...
        LogUtil.i(UIConstants.DEMO_TAG, "haveSDP->" + haveSDP);
        if (haveSDP != 1) {
            if (null != mCallNotification) {
    ...
    public void onCallGoing(TupCall tupCall) {
        LogUtil.i(UIConstants.DEMO_TAG, "onCallGoing");
        if (null == tupCall) {
    ...
    public void onCallRTPCreated(TupCall tupCall) {
        LogUtil.i(UIConstants.DEMO_TAG, "onCallRTPCreated");
        if (null == tupCall) {
            LogUtil.i(UIConstants.DEMO_TAG, "tupCall obj is null");
            return;
        }
    ...
    public void onCallConnected(TupCall call) {
        LogUtil.i(UIConstants.DEMO_TAG, "onCallConnected");
        if (null == call) {
            LogUtil.i(UIConstants.DEMO_TAG, "call obj is null");
            return;
        }
    ...
    public void onCallEnded(TupCall call) {
        LogUtil.i(UIConstants.DEMO_TAG, "onCallEnded");
        if (null == call) {
            LogUtil.i(UIConstants.DEMO_TAG, "onCallEnded call is null");
            return;
        }
    ...
    public void onCallDestroy(TupCall call) {
        LogUtil.i(UIConstants.DEMO_TAG, "onCallDestroy");

        Session callSessionByCallID = getCallSessionByCallID(call.getCallId());

        if (null == callSessionByCallID) {
            return;
        }
        ...
        Session callSession = getCallSessionByCallID(call.getCallId());
        if (callSession == null) {
            LogUtil.i(UIConstants.DEMO_TAG, "call session obj is null");
            return;
        }
