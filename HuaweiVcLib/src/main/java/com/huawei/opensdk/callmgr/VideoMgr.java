...
public class VideoMgr {
    ...
    private int controlLocalCameraMode1(Session session, boolean isOpen) {
        ...
        if (isOpen) {
            ...
            result = tupCall.setCaptureRotation(CallConstant.FRONT_CAMERA, 0);
            if (result != 0) {
            ...
            //TODO:采用发送默认图版本方式，替代关闭摄相头动作,必须是BMP格式
    ...
    public int setVideoOrient(int callId, int cameraIndex) {
         ...
         */
        int result = callManager.setMboileVideoOrient(
                callId,
                cameraIndex,
                orient,
                portrait,
    ...
    private boolean setVideoWindow(int callId,
                                   VideoWndType videoType,
                                   int videoIndex,
                                   int displayType) {
        int result;
        if (callId == 0) {
            result = callManager.createVideoWindow(
                    videoType.getIndex(),
                    videoIndex,
                    displayType);
        } else {
    ...
    public void initVideoWindow(final int callId) {
        handler.post(new Runnable() {
            ...
            public void run() {
                ...
                boolean localSetVideo = setVideoWindow(
                        callId,
                        VideoWndType.local,
                        localVideoIndex,
                        displayType);
                ...
                boolean remoteSetVideo = setVideoWindow(
                        callId,
                        VideoWndType.remote,
                        remoteVideoIndex,
                        displayType);
    ...
    private class OrientationDetector {
         ...
         * 布局方向，默认为竖向布局
         */
        private int layoutDirect = LAYOUT_LANDSCAPE;
        ...
        public void autoOrientationAdjust(Object object, boolean isOpen) {
            if (isOpen) {
                ...
                //TODO:修改的地方
                //this.lastOrientation = ORIENTATION_LANDSCAPE_LEFT;
                //TODO:修改的地方
                this.lastOrientation = ORIENTATION_LANDSCAPE_RIGHT;
        ...
        private void updateRotation(boolean isForce) {
            ...
            if (isForce || deviceOrientation != lastOrientation) {
                //LogUtil.i(UIConstants.DEMO_TAG, "updateRotation");
                // 更新旋转角度, 包括摄相头和显示窗口
...
//        }
        ...
        //添加灵敏度
        private int getOrientation(int orientation) {
            if ((orientation < (45 - sensitivity) && orientation >= 0) || (orientation >= (315 + sensitivity) && orientation <= 360)) {
                return ORIENTATION_PORTRAIT_UP;
            } else if (orientation >= (45 + sensitivity) && orientation < (135 - sensitivity)) {
                return ORIENTATION_LANDSCAPE_RIGHT;
            } else if (orientation >= (135 + sensitivity) && orientation < (225 - sensitivity)) {
                return ORIENTATION_PORTRAIT_DOWN;
            } else if (orientation >= (225 + sensitivity) && orientation < (315 - sensitivity)) {
                return ORIENTATION_LANDSCAPE_LEFT;
            } else {
                return ORIENTATION_UNKNOWN;
            }
...
//                    break;
                ...
                default:
//                    cameraCaptureRotation = 0;//3倒立
//                    windowsDisplayRotation = 3;//3:远端正常,
//                    windowsDisplayRotation = 0;//1
                    break;
            }
...
////            LogUtil.i(UIConstants.DEMO_TAG, "setOrientationLandscapeLeft()-->" + isLayoutPortrait());
//            if (isLayoutPortrait()) {
//                //最初
////                cameraCaptureRotation = 0;
////                windowsDisplayRotation = 1;//横屏
//
//                //最后修改
////                cameraCaptureRotation = 3;//竖屏时摄像头正的
////                windowsDisplayRotation = 0;//竖屏时远端画面正的
////                cameraCaptureRotation = 3;//设备横屏时，3：头向右,2:头向下，0整除
//
//                cameraCaptureRotation = 0;//设备横屏时，头向右
//                windowsDisplayRotation = 0;//竖屏时远端画面正的
//            } else {
//                //TODO:会议时修改为1正常, cameraCaptureRotation = 0;  windowsDisplayRotation = 1;
////                cameraCaptureRotation = 0;
...
//            LogUtil.i(UIConstants.DEMO_TAG, "setOrientationLandscapeRight()-->" + isLayoutPortrait());
//
//            if (isLayoutPortrait()) {
//                // 前置后置摄像头旋转角度一致
...
//            LogUtil.i(UIConstants.DEMO_TAG, "setOrientationPortraitUp()-->" + isLayoutPortrait());
//
//            if (isLayoutPortrait()) {
//                if (VideoMgr.getInstance().getCurrentCameraIndex() == CallConstant.FRONT_CAMERA) {
//                    cameraCaptureRotation = 3;
//                    windowsDisplayRotation = 0;//0：方向横屏
//                } else {
//
//                    cameraCaptureRotation = 1;
//                    windowsDisplayRotation = 0;//0：方向横屏
...
//            LogUtil.i(UIConstants.DEMO_TAG, "setOrientationPortraitDown()-->" + isLayoutPortrait());
//
//            if (isLayoutPortrait()) {
//                if (VideoMgr.getInstance().getCurrentCameraIndex() == CallConstant.FRONT_CAMERA) {
//                    cameraCaptureRotation = 1;
...
//                cameraCaptureRotation = 2;//本地：2-倒立
                windowsDisplayRotation = 2;//远端：0-倒立
            } else {
                // 前置后置摄像头旋转角度一致
                cameraCaptureRotation = 2;
//                windowsDisplayRotation = 2;
...
//                    windowsDisplayRotation = 0;//0：方向横屏
                    ...
                    //todo：11月30号修改竖向正常
//                    cameraCaptureRotation = 3;//3倒立
                    windowsDisplayRotation = 1;//1
                } else {
//                    cameraCaptureRotation = 1;
//                    windowsDisplayRotation = 0;
//
                    cameraCaptureRotation = 1;
                    windowsDisplayRotation = 1;//0：方向横屏
                }
            } else {
                if (VideoMgr.getInstance().getCurrentCameraIndex() == CallConstant.FRONT_CAMERA) {
                    cameraCaptureRotation = 3;
...
//                    cameraCaptureRotation = 1;//摄像头，1:倒立
                    windowsDisplayRotation = 1;//远端，3倒立
                } else {
                    cameraCaptureRotation = 3;
                    windowsDisplayRotation = 0;
                }
...
//                    windowsDisplayRotation = 1;
        ...
        private void setRotation(int cameraCaptureRotation, int windowsDisplayRotation) {
            ...
            if (session != null) {
                //前置摄像头
                if (VideoMgr.getInstance().getCurrentCameraIndex() == CallConstant.FRONT_CAMERA) {
                    LogUtil.i(UIConstants.DEMO_TAG, "FRONT-->lastOrientation--》" + lastOrientation);

                    switch (lastOrientation) {
                        //竖屏象限（摄像头在上）
                        case ORIENTATION_PORTRAIT_UP:
                            //设置摄像头采集方向
                            session.setCaptureRotation(1, 3);
                            //近端画面显示角度
                            session.setDisplayRotation(VideoWndType.local, 3);
                            //远端画面始终横屏
                            session.setDisplayRotation(VideoWndType.remote, 0);
                            LogUtil.i(UIConstants.DEMO_TAG, "竖屏象限（摄像头在上）");
                            break;

                        //竖屏象限（摄像头在下）
                        case ORIENTATION_PORTRAIT_DOWN:
                            session.setCaptureRotation(1, 1);
                            session.setDisplayRotation(VideoWndType.local, 1);
                            //远端画面始终横屏
                            session.setDisplayRotation(VideoWndType.remote, 0);
                            LogUtil.i(UIConstants.DEMO_TAG, "竖屏象限（摄像头在下）");
                            break;
                         ...
                         */
                        case ORIENTATION_LANDSCAPE_LEFT:
                            session.setCaptureRotation(1, 0);
                            session.setDisplayRotation(VideoWndType.local, 0);
                            //远端画面始终横屏
                            session.setDisplayRotation(VideoWndType.remote, 0);

                            LogUtil.i(UIConstants.DEMO_TAG, "横屏象限（摄像头在左）");
                            break;

                        //横屏象限（摄像头在右）
                        case ORIENTATION_LANDSCAPE_RIGHT:
                            session.setCaptureRotation(1, 2);
                            session.setDisplayRotation(VideoWndType.local, 2);
                            //远端画面始终横屏
                            session.setDisplayRotation(VideoWndType.remote, 0);
                            LogUtil.i(UIConstants.DEMO_TAG, "横屏象限（摄像头在右）");
                            break;
                        ...
                        default:
                            ...
                            //远端画面始终横屏
                            session.setDisplayRotation(VideoWndType.remote, 0);
                            LogUtil.i(UIConstants.DEMO_TAG, "默认的");
                            break;
                    }

                } else if (VideoMgr.getInstance().getCurrentCameraIndex() == CallConstant.BACK_CAMERA) {
//                    LogUtil.i(UIConstants.DEMO_TAG, "BACK-->cameraCaptureRotation--》" + 1 + ",windowsDisplayRotation-->" + windowsDisplayRotation);
