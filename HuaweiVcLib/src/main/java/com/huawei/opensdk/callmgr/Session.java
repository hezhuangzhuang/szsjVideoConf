...
public class Session {
    ...
    public boolean muteSpeak(boolean mute) {
        ...
        //true代表静音
        muteSpeakStatus = mute;
        return true;
    }

    private boolean muteSpeakStatus = false;//true:代表静音

    public boolean getMuteSpeakStatus() {
        return muteSpeakStatus;
    }

    public void setMuteSpeakStatus(boolean muteSpeakStatus) {
        this.muteSpeakStatus = muteSpeakStatus;
    }
    ...
    public boolean setCaptureRotation(int index, int rotation) {
        ...
        if (1 == index) {
            ...
            //TODO：修改显示模式   (0:拉伸模式 1:(不拉伸)黑边模式 2:(不拉伸)裁剪模式)
