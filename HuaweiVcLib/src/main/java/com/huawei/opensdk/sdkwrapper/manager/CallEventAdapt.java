...
class CallEventAdapt implements TupCallNotify {
    ...
    private void registerResultProc(TupRegisterResult tupRegisterResult) {
        ...
        switch (regState) {
            case TupCallParam.CALL_E_REG_STATE.CALL_E_REG_STATE_UNREGISTER:
                Log.i(UIConstants.DEMO_TAG, "CALL_E_REG_STATE.CALL_E_REG_STATE_UNREGISTER");
            ...
            case TupCallParam.CALL_E_REG_STATE.CALL_E_REG_STATE_REGISTERING:
                Log.i(UIConstants.DEMO_TAG, "CALL_E_REG_STATE.CALL_E_REG_STATE_REGISTERING");
            ...
            case TupCallParam.CALL_E_REG_STATE.CALL_E_REG_STATE_DEREGISTERING:
                Log.i(UIConstants.DEMO_TAG, "CALL_E_REG_STATE.CALL_E_REG_STATE_DEREGISTERING");
            ...
            case TupCallParam.CALL_E_REG_STATE.CALL_E_REG_STATE_REGISTERED:
                Log.i(UIConstants.DEMO_TAG, "CALL_E_REG_STATE.CALL_E_REG_STATE_REGISTERED");
            ...
            case TupCallParam.CALL_E_REG_STATE.CALL_E_REG_STATE_BUTT:
                Log.i(UIConstants.DEMO_TAG, "CALL_E_REG_STATE.CALL_E_REG_STATE_BUTT");
    ...
    /**
     * tupRegisterResult.getReasonCode()
     * 0：未注册
     * <p>
     * 1：注册中
     * <p>
    ...
    public void onRegisterResult(TupRegisterResult tupRegisterResult) {
        SPStaticUtils.put(UIConstants.REGISTER_RESULT_TEMP, tupRegisterResult.getRegState());

        //TODO:断网之后会收到的回调
        if (tupRegisterResult.getReasonCode() == 834) {
            //打印日志
            LogUtils.d("onRegisterResult-->华为平台监听到网络断了");
            SPStaticUtils.put(UIConstants.REGISTER_RESULT, tupRegisterResult.getReasonCode());
...
//            }
        ...
        //如果登录状态为失效
        if (3 != tupRegisterResult.getRegState() && 1 != tupRegisterResult.getRegState()) {
            boolean isLogOut = false;
            try {
                isLogOut = SPStaticUtils.getBoolean(UIConstants.IS_LOGOUT, false);
            } catch (Exception e) {
            }
            //没有调用登出接口
            if (!isLogOut) {
                //如果网络连接成功
//                if (NetworkUtil.isNetworkAvailable(LocContext.getContext())) {
//                    if (1 == tupRegisterResult.getRegState()) {
//                        LoginDialogActivity.startActivity(LocContext.getContext());
//                    }
//                }
            }
        }
        LogUtil.i(UIConstants.DEMO_TAG, "onRegisterResult: ReasonCode" + tupRegisterResult.getReasonCode() + ",RegState" + tupRegisterResult.getRegState());
