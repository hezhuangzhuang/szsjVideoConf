...
public class NotificationUtils {
    ...
    public static boolean areNotificationsEnabled() {
        return NotificationManagerCompat.from(BaseApp.context).areNotificationsEnabled();
    }
    ...
    public static void notify(String tag, int id, ChannelConfig channelConfig, Func1<Void, NotificationCompat.Builder> func1) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            NotificationManager nm = (NotificationManager) BaseApp.context.getSystemService(Context.NOTIFICATION_SERVICE);
            nm.createNotificationChannel(channelConfig.getNotificationChannel());
        }

        NotificationManagerCompat nmc = NotificationManagerCompat.from(BaseApp.context);

        NotificationCompat.Builder builder = new NotificationCompat.Builder(BaseApp.context);
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
    ...
    public static void cancel(String tag, final int id) {
        NotificationManagerCompat.from(BaseApp.context).cancel(tag, id);
    }
    ...
    public static void cancel(final int id) {
        NotificationManagerCompat.from(BaseApp.context).cancel(id);
    }
    ...
    public static void cancelAll() {
        NotificationManagerCompat.from(BaseApp.context).cancelAll();
    }
    ...
    public static class ChannelConfig {
        public static final ChannelConfig DEFAULT_CHANNEL_CONFIG = new ChannelConfig(
                BaseApp.context.getPackageName(), BaseApp.context.getPackageName(), IMPORTANCE_DEFAULT
        );
