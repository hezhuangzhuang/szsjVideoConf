...
object BarUtils {
    ...
    fun setStatusBarColor4Drawer(
        ...
        var i = 0
        val count = drawer.getChildCount()
        while (i < count) {
    ...
    fun getActionBarHeight(): Int {
        val tv = TypedValue()
        return if (BaseApp.context.getTheme().resolveAttribute(
    ...
    private fun invokePanels(methodName: String) {
        try {
            ...
            val service = BaseApp.context.getSystemService("statusbar")
    ...
    fun isSupportNavBar(): Boolean {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
            val wm = BaseApp.context.getSystemService(Context.WINDOW_SERVICE) as WindowManager
                ?: return false
        ...
        val menu = ViewConfiguration.get(BaseApp.context).hasPermanentMenuKey()
