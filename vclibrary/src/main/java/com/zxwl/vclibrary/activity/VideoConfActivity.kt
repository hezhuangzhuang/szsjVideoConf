...
import kotlinx.android.synthetic.main.activity_video_conf.*
 ...
 *  视频会议界面
 */
class VideoConfActivity : BaseVcActivity(), View.OnClickListener, CancelAdapt {
    ...
    //通话接听的时间
    private var videoAnswerTime: Long = 0
    //屏幕方向，1:竖屏，2:横屏
    private var mOrientation = Configuration.ORIENTATION_LANDSCAPE
    //是否有刷新过view
    private var isRefreshView = false
    //摄像头是否关闭，true：摄像头关闭，false：摄像头打开
    private var isLocalCameraClose = false
        ...
        LocBroadcastReceiver { broadcastName, obj ->
            ...
            when (broadcastName) {
                CustomBroadcastConstants.CONF_STATE_UPDATE -> {
                    ...
                    LogUtil.i(TAG, "会议信息更新：$confBaseInfo")
    ...
    private val mHandler = object : Handler() {
        override fun handleMessage(msg: Message) {
            ...
            when (msg.what) {
                //添加view
                ADD_LOCAL_VIEW -> setVideoContainer(
                    this@VideoConfActivity,
                    smallLocalView,
                    bigRemoteView,
                    hideVideoView
                )
                ...
                REFRESH_SELF_MEMBER -> {
                    ...
                    for (member in memberList) {
                        if (member.isSelf) {
                            //设置自己
                            MeetingMgr.getInstance().self = member
...
//                    tvConfName.setText(msg.obj as String)
    ...
    //摄像头方向
    private var mCameraIndex = CallConstant.FRONT_CAMERA
    private val NOT_ALPHA = 255
    ...
    override fun initData() {
        ...
        NotificationUtils.sendNotif(
            callInfo,
            NotificationUtils.VIDEO_ID,
            IntentConstant.VIDEO_CONF_MANAGER_ACTIVITY_ACTION,
            false
        )
    ...
    private fun itemControlClick(view: View, position: Int) {
        ...
        when (view.tag) {
            ...
            ivItemBroadcast.tag -> {
                ...
                broadcastAttendee(item!!, !item.isBroadcastSelf)
            }
...
//                }
    ...
    /**
     * 广播与会者
     * @param isBroadcast  true:广播，false:取消广播
     */
    private fun broadcastAttendee(member: Member, isBroadcast: Boolean) {
        val result = MeetingMgr.getInstance().broadcastAttendee(member, isBroadcast)
        if (0 != result) {
            ToastHelper.showShort("${if (isBroadcast) "广播" else "取消广播"}${member.displayName}失败")
            result
        }
        ToastHelper.showShort("${if (isBroadcast) "广播" else "取消广播"}${member.displayName}成功")
    }

    /**
     * 观看与会者
     */
    private fun watchAttendee(member: Member) {
        val result = MeetingMgr.getInstance().watchAttendee(member)
        if (0 != result) {
            ToastHelper.showShort("观看${member.displayName}失败")
            result
        }
        ToastHelper.showShort("观看${member.displayName}成功")
    }
    ...
    override fun setListener() {
        ...
        tvOpenSamll.setOnClickListener(this)
     ...
     * 会议主界面按钮点击事件
     */
    private fun videoLayoutClick(v: View?) {
        when (v) {
            //显示隐藏控制栏
            ivBg -> {
                switchShowControl()
    ...
    override fun getLayoutId(): Int = R.layout.activity_video_conf

    private fun finishActivity() {
        finishAndRemoveTask()
    }

    private fun videoDestroy() {
        if (null != CallMgr.getInstance().videoDevice) {
            LogUtil.i(TAG, "onCallClosed destroy.")
    ...
    override fun onResume() {
        ...
        setVideoContainer(this, smallLocalView, bigRemoteView, hideVideoView)

        setAutoRotation(this, true, mOrientation)
    }
    ...
    override fun onDestroy() {
        ...
        setAutoRotation(this, false, mOrientation)
     ...
     */
    fun closeSmallClick() {
        flLocal.isVisible = !flLocal.isVisible
        getLocalVideoView().isVisible = !getLocalVideoView().isVisible

        if (flLocal.isVisible) {
            tvOpenSamll.setCompoundDrawablesWithIntrinsicBounds(0, R.mipmap.ic_close_samll, 0, 0)
        } else {
            tvOpenSamll.setCompoundDrawablesWithIntrinsicBounds(0, R.mipmap.ic_open_samll, 0, 0)
        }
    }

    fun getHideVideoView(): SurfaceView {
        return VideoMgr.getInstance().localHideView
    }

    fun getLocalVideoView(): SurfaceView {
        return VideoMgr.getInstance().localVideoView
    }

    fun getRemoteVideoView(): SurfaceView {
        return VideoMgr.getInstance().remoteVideoView
    }

    fun setVideoContainer(
        context: Context,
        smallLayout: ViewGroup?,
        bigLayout: ViewGroup?,
    ...
    private fun addSurfaceView(container: ViewGroup, child: SurfaceView?) {
        if (child == null) {
            return
        }

        if (child.parent != null) {
            val vGroup = child.parent as ViewGroup
    ...
    fun setAutoRotation(`object`: Any, isOpen: Boolean, orientation: Int) {
    ...
    private fun releseChairManResult(msg: Message) {
        ...
        ivRequestChair.setImageResource(R.mipmap.ic_request_chairman)
    ...
    private fun requestChairManResult(msg: Message) {
        ...
        ivRequestChair.setImageResource(R.mipmap.ic_release_chairman)
    ...
    private fun refreshSelfMemberStatus(selfMember: Member) {
        ...
        ivRequestChair.setImageResource(if (isChairMan) R.mipmap.ic_release_chairman else R.mipmap.ic_request_chairman)
...
//        getViewAlphaAnimator(llBottomControl, 0f).start()
    ...
    fun switchCamera() {
        ...
        val callID = MeetingMgr.getInstance().currentConferenceCallID
        if (callID == 0) {
            return
        }
        mCameraIndex = if (CallConstant.FRONT_CAMERA == mCameraIndex)
            CallConstant.BACK_CAMERA
        else
            CallConstant.FRONT_CAMERA
        CallMgr.getInstance().switchCamera(callID, mCameraIndex)

        tvSwitchCamera.setCompoundDrawablesWithIntrinsicBounds(
            0,
            if (mCameraIndex == CallConstant.FRONT_CAMERA) R.mipmap.ic_switch_camera_normal else R.mipmap.ic_switch_camera_active,
            0,
            0
        )
    }
    ...
    private fun closeCamera() {
        ...
        if (isLocalCameraClose) {
            ...
            tvCloseCamera.setCompoundDrawablesWithIntrinsicBounds(
                0,
                R.mipmap.ic_close_camera,
                0,
                0
            )
        } else {
    ...
    private fun initTimeSubscribe() {
        videoAnswerTime = CallMgr.getInstance().videoAnswerTime
        if (0L == videoAnswerTime) {
            videoAnswerTime = System.currentTimeMillis()
        }
        subscribe = Flowable
            ...
            .subscribe {
                val confDuration = System.currentTimeMillis() - videoAnswerTime
    ...
    companion object {
        ...
        val TAG = VideoConfActivity::class.java.simpleName
