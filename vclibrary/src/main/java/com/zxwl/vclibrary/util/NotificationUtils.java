...
public class NotificationUtils {
    ...
    //语音通话时的notifId
    public static final int VOICE_ID = 0X101;
    ...
    public static boolean areNotificationsEnabled() {
        return NotificationManagerCompat.from(LocContext.getContext()).areNotificationsEnabled();
    }
...
//        if (Build.VERSION.SDK_INT >=26) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            NotificationManager nm = (NotificationManager) LocContext.getContext().getSystemService(Context.NOTIFICATION_SERVICE);
            nm.createNotificationChannel(channelConfig.getNotificationChannel());
        }

        NotificationManagerCompat nmc = NotificationManagerCompat.from(LocContext.getContext());

        NotificationCompat.Builder builder = new NotificationCompat.Builder(LocContext.getContext());
//        if (Build.VERSION.SDK_INT >= 26) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
    ...
    public static void cancel(String tag, final int id) {
        NotificationManagerCompat.from(LocContext.getContext()).cancel(tag, id);
    }
    ...
    public static void cancel(final int id) {
        NotificationManagerCompat.from(LocContext.getContext()).cancel(id);
    }
    ...
    public static void cancelAll() {
        NotificationManagerCompat.from(LocContext.getContext()).cancelAll();
    }
    ...
    private static void invokePanels(final String methodName) {
        try {
            ...
            Object service = LocContext.getContext().getSystemService("statusbar");
    ...
    public static class ChannelConfig {
        public static final ChannelConfig DEFAULT_CHANNEL_CONFIG = new ChannelConfig(
                LocContext.getContext().getPackageName(), LocContext.getContext().getPackageName(), IMPORTANCE_DEFAULT
        );
...
//            if (Build.VERSION.SDK_INT >=26) {
     ...
     * @param calling
     */
    public static void sendNotif(final CallInfo callInfo,
                                 int notifyId,
                                 final String action,
                                 final boolean calling) {
        NotificationUtils.notify(notifyId, new NotificationUtils.Func1<Void, NotificationCompat.Builder>() {
            @Override
            public Void call(NotificationCompat.Builder param) {
                ...
                String isVideoString = callInfo.isVideoCall() ? "视频" : "语音";
                String callingString = calling ? "呼叫中" : "通话中";

                //判断是否是会议
                param
                        .setSmallIcon(R.mipmap.ic_open_louder)
                        .setColor(Color.parseColor("#be002e"))
                        //右边的按钮
//                        .setLargeIcon(BitmapFactory.decodeResource(LocContext.getContext().getResources(), R.mipmap.ic_zt_logo))
                        .setContentTitle(LocContext.getContext().getResources().getString(R.string.app_name))
                        .setContentText(isVideoString + callingString + ",点击以继续")
