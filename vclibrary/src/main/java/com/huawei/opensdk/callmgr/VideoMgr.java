...
//            String picturePath = Environment.getExternalStorageDirectory() + File.separator + Constants.BMP_FILE;
            String picturePath = LocContext.getContext().getExternalFilesDir(Environment.DIRECTORY_PICTURES) + File.separator + Constants.BMP_FILE;
     ...
     * @param callId 呼叫id
     */
    public void initVideoWindow(final int callId, int line) {
        LogUtil.i(TAG, "initVideoWindow() enter" + callId + " ,line：" + line);
...
//            }
        ...
        private void setRotation(int cameraCaptureRotation, int windowsDisplayRotation) {
            ...
            LogUtil.i(TAG, "摄像头采集方向-cameraCaptureRotation:" + cameraCaptureRotation + ",窗口显示方向-windowsDisplayRotation:" + windowsDisplayRotation);
            if (VideoMgr.getInstance().getCurrentCameraIndex() == CallConstant.FRONT_CAMERA) {
                // 1表示前置摄像头
