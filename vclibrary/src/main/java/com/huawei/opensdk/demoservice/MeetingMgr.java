...
public class MeetingMgr implements IMeetingMgr {
    ...
    public void setConfBaseInfoList(List<ConfBaseInfo> confBaseInfoList) {
        MeetingMgr.confBaseInfoList = confBaseInfoList;
    }
...
//                if ("10087".equals(attendee.getStatusInfo().getParticipantId()))
//                {
//                    member.setSelf(true);
//                    this.setSelf(member);
//                }
...
//        }
    ...
    public int bookConference(BookConferenceInfo bookConferenceInfo) {
        ...
        bookConfInfo.setSize(bookConferenceInfo.getMemberList().size());
    ...
    public int joinConf(TsdkConfJoinParam confJoinParam, boolean isVideo, String joinNumber) {
        ...
        List<ConfBaseInfo> confBaseInfoList = MeetingMgr.getInstance().getConfBaseInfoList();

//        if (confBaseInfoList == null || confBaseInfoList.size() < 1) {
//            return -1;
//        }
//        ConfBaseInfo confBaseInfo = confBaseInfoList.get(MeetingMgr.getInstance().getPosition());
//        if (confBaseInfo == null) {
//            return -1;
//        }
        confBaseInfo.setMediaType(TsdkConfMediaType.TSDK_E_CONF_MEDIA_VIDEO);
//        String accessNumber = confBaseInfo.getAccessNumber();
//        confJoinParam.setAccessNumber(accessNumber);
        switch (confBaseInfo.getMediaType()) {
    ...
    public int leaveConf() {
        ...
        LogUtil.i(TAG, "test leaveConf. result=" + result);
        if (result == 0) {
            currentConference = null;
    ...
    public int endConf() {
        ...
        LogUtil.i(TAG, "test endConf.result=" + result);
        if (result == 0) {
            currentConference = null;
    ...
    public int hangupAttendee(Member attendee) {
        ...
        LogUtil.i(TAG, "hangupAttendee. result =" + result);
        return result;
    }
    ...
    public int redialAttendee(Member attendee) {
        ...
        LogUtil.i(TAG, " redialAttendee. result =" + result);
    ...
    public int muteConf(boolean isMute) {
        ...
        return currentConference.muteConference(isMute);
    }
...
//        }
        ...
        return currentConference.releaseChairman();
    }
    ...
    public int requestChairman(String chairmanPassword) {
        ...
        LogUtil.i(TAG, "requestChairman. result = " + result);
        return result;
    }
    ...
    public int requestPresenter() {
        if (null == currentConference) {
            Log.e(TAG, "request presenter failed, currentConference is null ");
            return -1;
        }
        LogUtil.i(TAG, "begin request presenter");
        int result = currentConference.requestPresenter();
        return result;
    }
     ...
     * @return int
     */
    public int applySpeak(boolean isApplySpeak) {
        if (null == currentConference) {
            Log.e(TAG, "request presenter failed, currentConference is null ");
            return -1;
        }
        LogUtil.i(TAG, "begin request applySpeak");
        int result = currentConference.requestSpeaking(isApplySpeak);
        return result;
    }
     ...
     * @return
     */
    public int rollCall(Member attendee, boolean isRollCall) {
        if (null == currentConference) {
        ...
        if (isRollCall) {
            result = currentConference.rollCallAttendee(attendee.getNumber());
        } else {
            result = currentConference.rollCallAttendee(null);
        }
    ...
    public int joinConferenceByAnonymous(String confId, String displayName,
                                         ...
                                         String servicePort, boolean isVPN, String randomNum) {
        ...
        //add by lisen
        anonymousParam.setAuthType(TsdkConfAnonymousAuthType.TSDK_E_CONF_ANONYMOUS_AUTH_RANDOM);
        anonymousParam.setRandom(randomNum);
    ...
    public int joinDataConf() {
        ...
        LogUtil.i(TAG, "joinDataConf. result = " + result);
    ...
    public void sendConfData(int dataType, String message) {
        ...
        LogUtil.i(TAG, "chatMsgInfo.getDataType():" + chatMsgInfo.getDataType() + "   chatMsg:" + chatMsgInfo.getChatMsg().toString());
     ...
     * @param commonResult
     * @param tsdkJoinConfIndInfo //TODO：正常流程应走到这
     */
    public void handleJoinConfResult(TsdkConference tsdkConference, TsdkCommonResult commonResult, TsdkJoinConfIndInfo tsdkJoinConfIndInfo) {
        LogUtil.i(TAG, "handleJoinConfResult.加入会议成功");
        ...
        if (result == 0) {
            ...
            if (null != tsdkCall) {
                ...
                if (tsdkCall.getCallInfo().getIsVideoCall() == 1) {
                    VideoMgr.getInstance().initVideoWindow(tsdkCall.getCallInfo().getCallId(),1536);
                }
            }
    ...
    public void handleConfctrlOperationResult(TsdkConference conference, TsdkConfOperationResult result) {
        ...
        LogUtil.i(TAG, "handleConfctrlOperationResult: " + ret);
        if (ret != 0) {
        ...
        switch (TsdkConfOperationType.enumOf(result.getOperationType())) {
            ...
            case TSDK_E_CONF_POSTPONE_CONF:
                mConfNotification.onConfEventNotify(ConfConstant.CONF_EVENT.POSTPONE_CONF_RESULT, ret);
                break;
            //申请主讲人
    ...
    /**
     * This method is used to handle receive chat msg notify in conf.
     * 处理接收到的聊天消息
     *
     * @param confChatMsgInfo Indicates chat msg info.
     *                        聊天信息
     */
