...
//                        intent.putExtra(UIConstants.PEER_NUMBER, callInfo.peerNumber);
                ...
                case JOIN_VIDEO_CONF_SUCCESS:
                    if (msg.obj instanceof String) {
                        if (null != CallMgr.getInstance()) {
                            CallMgr.getInstance().setVideoAnswerTime(System.currentTimeMillis());
                        }
    ...
    public void onConfEventNotify(ConfConstant.CONF_EVENT confEvent, Object params) {
        switch (confEvent) {
            ...
            case REQUEST_PRESENTER_RESULT:
                LocBroadcast.getInstance().sendBroadcast(CustomBroadcastConstants.REQUEST_PRESENTER_RESULT, params);
                break;
...
//                LocBroadcast.getInstance().sendBroadcast(CustomBroadcastConstants.GET_DATA_CONF_PARAM_RESULT, params);
                ...
                if (result != 0) {
                    ToastHelper.showShort("获取数据会议参数失败");
                    return;
                }
