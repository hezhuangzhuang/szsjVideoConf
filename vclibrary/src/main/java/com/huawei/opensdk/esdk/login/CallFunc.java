...
//                        mFilePath = Environment.getExternalStorageDirectory() + File.separator + RINGING_FILE;
                        mFilePath = LocContext.getContext().getExternalFilesDir(Environment.DIRECTORY_MUSIC) + File.separator + RINGING_FILE;
                        ...
                        NotificationUtils.sendNotif(callInfo,
                                NotificationUtils.CALL_IN_ID,
                                IntentConstant.CALL_IN_ACTIVITY_ACTION,
                                true);
                    }
                    break;
    ...
    private Handler mHandler = new Handler(Looper.getMainLooper()) {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case UPGRADE_FAILED:
                    ToastHelper.showShort("对方拒绝了您升级到视频通话的请求");
                    break;

                case CTD_FAILED:
                    ToastHelper.showShort("CTD call failed");
                    break;

                case CTD_SUCCESS:
                    ToastHelper.showShort("CTD call success");
                    break;
...
//                    mFilePath = Environment.getExternalStorageDirectory() + File.separator + RING_BACK_FILE;
                    mFilePath = LocContext.getContext().getExternalFilesDir(Environment.DIRECTORY_MUSIC) + File.separator + RING_BACK_FILE;
