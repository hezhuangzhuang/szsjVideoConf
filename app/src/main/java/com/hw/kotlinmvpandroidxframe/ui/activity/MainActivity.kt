...
class MainActivity : BaseMvpActivity<ConfPresenter>(), MainContract.View {
    ...
    private val loginReceiver = object : LocBroadcastReceiver {
        override fun onReceive(broadcastName: String?, obj: Any?) {
            LogUtils.i("loginReceiver-->$broadcastName")
            when (broadcastName) {
                CustomBroadcastConstants.LOGIN_SUCCESS -> {
                    dismissLoading()
...
//                    val errorMessage = obj as String
    ...
    override fun authenticationFail(errorMsg: String) {
        ToastHelper.showShort("鉴权失败:${errorMsg}")

        showErrorDialog("鉴权失败", AUTHENTICATION_FAIL)
    }
     ...
     * 查询会议成功
     */
    override fun queryConfSuccess(confBeans: List<ConfBean>) {
        refreshLayout.finishRefresh(true)
        confAdapter.replaceData(confBeans)
    }
    ...
    private fun getIntentData(intent: Intent?) {
        ...
        intent?.apply {
            otherStart = getBooleanExtra(RouterPath.Huawei.FILED_OTHER_START, false)
            //第三方启动
            if (otherStart) {
                appPackageName = getStringExtra(
                    RouterPath.Huawei.FILED_APP_PACKAGE_NAME
                )

                secretKey = getStringExtra(
                ...
                siteUri = getStringExtra(RouterPath.Huawei.FILED_USER_NAME)
                displayName = getStringExtra(RouterPath.Huawei.FILED_DISPLAY_NAME)
                passWord = getStringExtra(RouterPath.Huawei.FILED_PASS_WORD)
                smcUrl = getStringExtra(RouterPath.Huawei.FILED_HUAWEI_SMC_URL)
                smcPort = getStringExtra(RouterPath.Huawei.FILED_HUAWEI_SMC_PORT)
            ...
            .callback(object : PermissionUtils.FullCallback {
                override fun onGranted(permissionsGranted: List<String>) {
                    ...
                    if (otherStart) {
                        ...
                        //初始化华为
                        HuaweiLoginImp.getInstance()
                            .initHuawei(BaseApp.context as Application?, BuildConfig.APPLICATION_ID)
...
//        SPStaticUtils.put(UserContants.USER_NAME, userInfo.data.accountName)
    ...
    private fun initAdapter() {
        ...
        confAdapter.setOnItemChildClickListener { adapter, view, position ->
            ...
            HuaweiModuleService.joinConf(dateBean!!.accessCode)
        }
    ...
    private fun handlerHuaweiLoginSuccess() {
        ...
        when (startType) {
            //创建会议
            RouterPath.Huawei.TYPE_CREATE_CONF -> {
                LogUtils.i("会议名称:${confName},sites:${sites}")
                ...
                val split = sites?.split(",")

                //创建视频会议
                HuaweiModuleService.createConf(
                    confName!!, split!!, true
                )
            }
            ...
            RouterPath.Huawei.TYPE_JOIN_CONF -> {
                //加入会议
                HuaweiModuleService.joinConf(smcConfId!!)
            }
    ...
    private fun handlerHuaweiLoginFail() {
        showErrorDialog("登录华为平台失败", HUAWEI_LOGIN_FAIL)
    }
